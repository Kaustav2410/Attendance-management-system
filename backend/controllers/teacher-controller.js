...
const teacherLogIn = async (req, res) => {
    try {
        ...
        if (user) {
            ...
            if (validated) {
                subjects=await user.populate("teachSubject", "subName sessions")
                classes=await user.populate("teachSclass")
            ...
            return res.status(401).json({
                success:false,
                message:`Teacher with the given details doesn't exists`
            })
        }
    } 
    catch (err) {
